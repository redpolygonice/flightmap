#include "roaddevice.h"
#include "data/mission.h"

namespace device
{

bool RoadDevice::Send(const mavlink_message_t &message)
{
...
bool RoadDevice::CreateMission(const data::CoordinateList &points, data::CoordinateList &mission, const common::AnyMap &params)
{
	if (_mission == nullptr)
...
void RoadDevice::WriteMission(const data::CoordinateList &points, const common::AnyMap &params)
{
	if (_mission == nullptr)
		return;
...
void RoadDevice::ReadMission(data::CoordinateList &points)
{
	if (_mission == nullptr)
		_mission = data::Mission::Create(shared_from_this());
...
void RoadDevice::SendCommand(uint16_t cmd, float param1, float param2, float param3, float param4, float param5, float param6, float param7)
{
...
void RoadDevice::SendCommandInt(uint16_t cmd, float param1, float param2, float param3, float param4, float x, float y, float z)
{
	mavlink_message_t data;
	memset(&data, 0, sizeof(data));
...
void RoadDevice::RcChannelsOverride(uint16_t ch1, uint16_t ch2, uint16_t ch3, uint16_t ch4)
{
	mavlink_message_t data;
	memset(&data, 0, sizeof(data));
...
void RoadDevice::SetMode(uint32_t mode)
{
	mavlink_message_t data;
	memset(&data, 0, sizeof(data));
...
void RoadDevice::SetHome(float lat, float lon, float alt)
{
	mavlink_message_t data;
	memset(&data, 0, sizeof(data));
...
void RoadDevice::SetPosition(double lat, double lon, double alt)
{
